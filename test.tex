...
vector<node> e;
ll boruvka(const vector<node> &e, int n, int m)
{
    for (int i = 1; i <= n; ++i)
        fa[i] = i;
    vector<int> id(n + 5, -1);
    ...
    while (flag) {
        ...
        for (int i = 0; i < m; ++i) {
            ...
            if (x != y) {
                if (id[x] == -1 || e[id[x]].w > e[i].w)
                    id[x] = i;
                if (id[y] == -1 || e[id[y]].w > e[i].w)
                    id[y] = i;
            }
        }
        for (int i = 1; i <= n; ++i)
            if (get(i) == i && id[i] != -1 && get(e[id[i]].u) != get(e[id[i]].v)) {
                merge(e[id[i]].u, e[id[i]].v);
                ans += e[id[i]].w;
                // cout << id[i] << '\n';
                --cnt;
                flag = true;
...
int lca(int x, int y)
{
    ...
    if (l > r)
        swap(l, r);
    int k = log2(r - l + 1);
    return euler[min(f[k][l], f[k][r - (1 << k) + 1])];
}
\end{lstlisting}
...
//完美消除序列：一个点的序列满足 vi 在 {vi,...,vn} 的导出子图中为一个单纯点
//最小染色：用最少的颜色给点染色是的所有边连接的两点颜色不同
//最大独立集：最大的点集使得点集中任意两点都没有边直接相连
//最小团覆盖：用最少的团覆盖所有的点
//弦图的色数/团数：按照完美消除序列从后往前依次给每个点染色，给每个点染上可以染的最小颜色。只需要求色数时，只需要将最大label+1即可
...
void tarjan(int x, int fa)
{
    ...
    for (int i = head[x]; i; i = Next[i]) {
        ...
        if (!dfn[y]) {
            ...
            if (low[y] >= dfn[x]) {
                vbcc.push_back(vector<int>());

                while (stk.top() != y) {
                    vbcc.back().push_back(stk.top());
                    stk.pop();
                }
                vbcc.back().push_back(y);
                stk.pop();

                vbcc.back().push_back(x);
            }
        }
        else if (y != fa)
            low[x] = min(low[x], dfn[y]);
...
\begin{lstlisting}
//最终匹配一定要是完美匹配，可以通过初始化负无穷来判断
//本模板通过添加虚点与虚边构建完全图，再计算需要计算的点的费用和
//最终匹配中满足 lx + ly = a
//匹配方案存在 match 数组中
//最大取正权，最小取负权
int n;
const int N = 505;
int a[N][N];
int posx[N], posy[N], pre[N];
int lx[N], ly[N], matchx[N], matchy[N];
int slack[N], d;
bool visx[N], visy[N];
queue<int> q;
int nl, nr; //左右的点数
...
\subsection{BSGS算法}
\begin{lstlisting}
int BSGS(int a, int n, int mod, int st = 1)
{
    unordered_map<int, int> mp;
    int m = ceill(sqrtl(mod));
    int s = 1;
    for (int i = 0; i < m; ++i) {
        mp[1ll * s * n % mod] = i;
        s = 1ll * s * a % mod;
...
int exBSGS(int a, int n, int mod)
{
    a %= mod, n %= mod;
    if (n == 1 || mod == 1)
        return 0;
...
//固定模数与原根，O(P^0.75/log^0.5 P)预处理，O(logP)查询
//查询的是a^x=n中的a，返回的是最小x
//先init，然后调用get即可，solve不需要知道
struct BSGS_O {
    ...
    void init(int _g, int _mod)
    {
        g = _g, mod = _mod, block = sqrtl(1ll * mod * sqrtl(mod) / logl(mod));
        mp.clear();
        tag = qpow(qpow(g, block, mod), mod - 2, mod);
        ...
        sz = sqrtl(mod) + 1;
        h.resize(sz + 5), p.resize(sz + 5), v.resize(sz + 5);
        h0 = solve(mod - 1);
        ...
        for (int i = 2; i <= sz; ++i) {
            ...
            for (int j = 1; j <= idx && p[j] <= sz / i; ++j) {
                h[i * p[j]] = (h[i] + h[p[j]]) % (mod - 1);
                v[i * p[j]] = true;
                if (i % p[j] == 0)
                    break;
            }
    ...
    int get(int x)
    {
        if (x <= sz)
            return h[x];
        int now = mod / x, r = mod % x;
        if (r < x - r)
            return ((h0 + get(r)) % (mod - 1) - h[now] + mod - 1) % (mod - 1);
        else
            return (get(x - r) - h[now + 1] + mod - 1) % (mod - 1);
    }
} bsgs;
\end{lstlisting}
...
ll ask_rk(ll x)
{
    ll rk = 1;
    ll temp = 0;
    for (int i = tot;i;--i) {  //tot 指rebuild后的维数
        int flag = cmp (x ^ temp, d[i]);
        if (flag == 1) {
            return (rk += qpow(2, i) - 1 + mod) %= mod;
        }
        else if (now < 0)
            continue;
        else {
...
void NTT(vector<int> &a, int n)
{
    ...
    for (int i = 1; i < n; i <<= 1) {
        for (int k = i & (1 << 19); k; --k)
            if (t[k] >= mod * 9ull)
                t[k] -= mod * 9ull;
        for (int j = 0, p = i << 1; j < n; j += p)
            for (int k = 0; k < i; ++k) {
                const int x = t[i + j + k] * gk[i + k] % mod;
                t[i + j + k] = t[k + j] + mod - x;
                t[k + j] += x;
            }
    }
...
int com_size(int n, int m)
{
    ...
    for (static int i = 1; i < sz; i <<= 1) {
        gk[i] = 1;
        const int w = qpow(g, mod / i / 2);
        for (int j = 1; j < i; ++j)
            gk[i + j] = 1ll * gk[i + j - 1] * w % mod;
    }
    for (int i = 0; i < sz; ++i)
...
\subsection{多项式快速幂}
\begin{lstlisting}
// 100003173198118 : 1297151213
poly operator*=(const int &t)
{
    auto &a = *this;
    for (auto &c : a)
        c = 1ll * c * t % mod;
...
poly pow(int k, int n)  // k < mod
{
    ...
    if (1ll * k * cnt >= n)
        return {0};
    return ((poly{qpow(tag, mod - 2)} * div_xk(cnt)).ln(n) * k).exp(n).mul_xk(1ll * k * cnt).mod_xk(n) * qpow(tag, k);
}
...
poly pow(const string &s, int n)  // k >= mod
{
    ...
    if (cnt && s.size() > 7)
        ...
        return ((poly{qpow(tag, mod - 2)} * div_xk(cnt)).ln(n) * k).exp(n).mul_xk(1ll * k * cnt).mod_xk(n) * qpow(tag, k1);
}
\end{lstlisting}
...
poly sqrt_k(int k, int n)  // f[0]是mod下k次剩余
{
    poly f(*this);
    int s = f[0], inv = qpow(s, mod - 2);
    for (auto &c : f)
        c = 1ll * c * inv % mod;
    f = f.ln(n);
    int p = qpow(k, mod - 2);
...
\begin{large}
    ...
    $\displaystyle \sum_{i=1}^{n}\sum_{j=1}^{m}\sigma(gcd(i,j))[\sigma\leq{a}]=\sum_{k=1}^{n}\lfloor \frac{n}{k}\rfloor\lfloor \frac{m}{k}\rfloor g(k)$\\$g(k)=\sum_{d|k}\sigma(d)\mu(\lfloor \frac{k}{d}\rfloor)[\sigma(d)\leq a]$\\
    $\displaystyle f(n)=\sum_{i=1}^{\sqrt{n}}\mu(i)\lfloor \frac{n}{i^2} \rfloor$\\
    $\displaystyle \sum_{i=1}^{n}\frac{1}{i}=ln(n)+r+\frac{1}{2n}$\\$r=0.57721566490153286060651209$\\
\end{large}
\\
