...
\begin{lstlisting}
namespace algebra {
static constexpr int limit = 250;
static constexpr int N = 400005;
static constexpr int mod = 998244353;
using ll = long long;
mt19937 rng(chrono::steady_clock::now().time_since_epoch().count());

inline int add(int a, int b, const int mod = mod) { return (a += b) >= mod ? a - mod : a; }
inline int sub(int a, int b, const int mod = mod) { return (a -= b) < 0 ? a + mod : a; }
...
void NTT(vector<int>& a, int n, int opt)
{
    init(n, a);
    for (int i = 0; i < n; ++i)
        if (i < rev[i])
    ...
    for (int i = (gk[0] = 1); i < n; i <<= 1) {
        int wn = qpow(g, (mod - 1) / (i << 1));
        if (opt == -1)
            wn = qpow(wn, mod - 2);
        for (int k = i - 2; k >= 0; k -= 2)
            gk[k + 1] = mul(gk[k] = gk[k >> 1], wn);
        for (int j = 0, p = i << 1; j < n; j += p) {
            for (int k = 0; k < i; ++k) {
                int x = a[j + k], y = mul(gk[k], a[j + k + i]);
...
poly() {}
poly(int a0) : a{ a0 } { normalize(); }
poly(const vector<int>& t) :a(t) { normalize(); }
...
{ // 取前k项
    return vector<int>(begin(a), begin(a) + min(k, a.size()));
}
...
{ // 除以 x^k 也就是丢弃前k项
    return vector<int>(begin(a) + min(k, a.size()), end(a));
}
...
poly operator *= (const poly& t) { ntt::mult(a, t.a);normalize(); return *this; }
poly operator * (const poly& t) const { return poly(*this) *= t; }
...
poly deriv(int k = 1)
{
    if (k > size())
        return 0;
    vector<int>f(a.size() - k);
    for (int i = k;i < a.size();++i)
        f[i - k] = mul(a[i], mul(frac(i), ifrac(i - 1)));
    return f;
}
...
poly integr()
{
    ...
    for (int i = 1;i <= a.size();++i)
        f[i] = mul(a[i - 1], algebra::qpow(i, mod - 2));
    return f;
}
...
poly exp(int n)
{
    if (n == 1)
        return { 1 };
    poly g = exp((n + 1) >> 1);
    return (g * (poly(1) - g.ln(n) + mod_xk(n))).mod_xk(n);
}
...
poly pow(int k, int n)  // k < mod
{
    ...
    if (1ll * k * cnt >= n)
        return { 0 };
    return (poly(k) * (poly(qpow(tag, mod - 2)) * div_xk(cnt)).ln(n)).exp(n).mul_xk(mul(k, cnt)).mod_xk(n) * poly(qpow(tag, k));
}
...
poly pow(const string& s, int n) // k >= mod
{
    ...
    int cnt = 0, tag = 0;
    for (const auto& c : s)
        k = add(mul(k, 10), c - '0'), k1 = add(mul(k1, 10, mod - 1), c - '0', mod - 1);
    if (all_of(a.begin(), a.end(), [](auto& p) {return p == 0;}))
        return { 0 };
    else {
        for (const auto& c : a)
    ...
    if (cnt && s.size() > 7)
        ...
        return (poly(k) * (poly(qpow(tag, mod - 2)) * div_xk(cnt)).ln(n)).exp(n).mul_xk(mul(k, cnt)).mod_xk(n) * poly(qpow(tag, k1));
}
};
};
